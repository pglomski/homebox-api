...
This module provides object-oriented interfaces for interacting with a Homebox server,
including management of locations, tags, and items. It supports importing and exporting
data via CSV, as well as a CLI for basic operations.

Classes:
    HomeboxClient: Interface to the Homebox API.
    Location: Represents a location object with methods to update or delete it.
    Tag: Represents a tag with methods for manipulation.
    Item: Represents an inventory item.

Functions:
    get_client: Instantiates a HomeboxClient from local credential files.
    load_locations_from_csv: Loads locations from a CSV file.
...
class Location:
    ...
    def delete(self) -> Any:
        """delete method."""
        res = requests.delete(
            f"{self.client.base_url}/locations/{self.id}", headers=self.client.headers
        )
    ...
    def set_parent(self, new_parent_name: str) -> Any:
        """set_parent method."""
...
class Label:
    ...
    def rename(self, new_name: str) -> Any:
        """rename method."""
        res = requests.put(
            f"{self.client.base_url}/tags/{self.id}",
            headers=self.client.headers,
            json={"name": new_name},
        )
        res.raise_for_status()
...
class Item:
    ...
    def delete(self) -> Any:
        """delete method."""
        res = requests.delete(
            f"{self.client.base_url}/items/{self.id}", headers=self.client.headers
        )
        res.raise_for_status()
...
class HomeboxClient:
    ...
    def __init__(self, base_url: Any, username: Any, password: Any) -> Any:
        """__init__ method."""
        self.base_url = base_url.rstrip("/")
        self.headers = {"Content-Type": "application/json"}
        self._authenticate(username, password)

    def _authenticate(self, username: Any, password: Any) -> Any:
    ...
    def resolve_location_path(self, path: PathStr) -> Optional[str]:
        """resolve_location_path method."""
        if not path:
            return None
    ...
    def resolve_tag_names(self, tag_names_str: TagId) -> List[str]:
        """resolve_tag_names method."""
    ...
    def build_location_lookup_tree(self) -> Dict[str, str]:
        """build_location_lookup_tree method."""
        all_locations = self.get_all_locations()
        by_id = {loc.id: loc for loc in all_locations}
        full_paths = {}

        def get_path(loc: Any) -> Any:
            """
            get_path method.
            :param loc:
            :type loc: Any
    ...
    def export_items_readable_csv(self, filepath: PathStr) -> None:
        """export_items_readable_csv method."""
...
def cli() -> Any:
    ...
    parser = argparse.ArgumentParser(
        description="Homebox CLI for managing locations, tags, and items."
    )
    ...
    if args.command == "create-location":
        if args.dry_run:
            logging.info(
                f"[DRY RUN] Would create location '{args.name}' with parent '{args.parent}'"
            )
        else:
        ...
        for loc in load_locations_from_csv(args.csv):
            if args.dry_run:
                logging.info(
                    f"[DRY RUN] Would create location '{loc['name']}' with parent '{loc['parent']}'"
                )
            else:
